...
class CAPMBot(Agent):
    ...
    def _submit_order(self, order: Order) -> bool:
        ...
        if self._reactive_pend:
            self.inform("REACTIVE eat-order pending, skipping new submission.")
            return False
        ...
        if key in self._pend_orders:
            self.inform(f"Pending order already exists for {order.market.name} [{side_key}], skipping.")
            return False
        ...
        if existing is not None and existing.fm_id in self._cancelling_order_ids:
            self.inform(f"Cancel pending for {order.market.name} [{side_key}], skipping re-post.")
            return False
    ...
    def _cancel_all_my_orders(self):
        ...
        for market_id in list(self._my_standing_orders.keys()):
            for side_key, order in list(self._my_standing_orders[market_id].items()):
                # Skip orders that are still pending confirmation (no valid fm_id yet).
    ...
    def _compute_marginal_value(self, market: Market) -> float:
        ...
        :param market: the Market whose asset's marginal value that I want.
    ...
    def order_accepted(self, order: Order):
        ...
        if order.order_type == OrderType.CANCEL:
            ...
            # If this was my reactive order, clear the pending flag.
    ...
    def _update_my_standing_orders(self):
        ...
        for order in Order.my_current().values():
            ...
            if order.units != 1:
                self.inform(
                    f"[Guard] Cancelling multi-unit order ({order.units} units) "
                ...
                if len(order_list) > 1:
                    ...
                    for o in order_list:
                        if o.fm_id != keep.fm_id:
                            self.inform(
                                f"[Guard] Cancelling duplicate {side_key} "
        ...
        for mid, sides in live.items():
            if "BUY" in sides and "SELL" in sides:
                for side_key, order_list in sides.items():
                    o = order_list[0]
                    self.inform(
                        f"[Guard] Cancelling conflicting {side_key} in {o.market.name}"
                    )
                    self._cancel_order(o)
                live[mid] = {}
    ...
    def _execute_trading_strategy(self):
        ...
        if not has_market_orders:
            self.inform("[Strategy] No standing orders in market, cancelling my orders and wait.")
    ...
    def _run_reactive(self, current_performance: float):
        ...
        if not candidates:
            ...
            self.inform("[REACTIVE] No profitable eat opportunities, switching to ACTIVE.")
            self._run_active(current_performance)
            return
    ...
    def _submit_reactive_order(
        ...
        if side == OrderSide.BUY:
            ...
            self.inform(
                f"[REACTIVE] BUY {market.name}: cash insufficient (need {price/100:.2f}, "
                f"have {self._holdings.cash/100:.2f}), trying shortage handler."
            )
    ...
    def _run_active(self, current_performance: float):
        ...
        for market in self.markets.values():
            ...
            if prefer_sell:
                ...
                if bid is None or sell_price > bid:
                    ...
                    self._cancel_active_slot(market_id, OrderSide.SELL, reason="sell_price <= bid, switched to REACTIVE")
            else:
                ...
                if ask is None or buy_price < ask:
                    self._manage_active_order(
                        market, OrderSide.BUY, buy_price, current_performance
                    )
                else:
                    # An ask exists but is not profitable; cancel any stale BUY.
                    self._cancel_active_slot(market_id, OrderSide.BUY, reason="buy_price >= ask, switched to REACTIVE")
    ...
    def _manage_active_order(self, market: Market, side: OrderSide, 
        ...
        # No existing order, check the cancel guard before posting.
        # An old order for this slot may still be in _cancelling_order_ids.
        old_order = self._my_standing_orders.get(market_id, {}).get(side_key)
        if old_order is not None and old_order.fm_id in self._cancelling_order_ids:
            self.inform(
                f"[ACTIVE] Cancel pending for {side_key} {market.name}, skipping repost."
            )
            return
        ...
        if side == OrderSide.BUY:
            if self._holdings.cash_available < target_price:
                self.inform(
                    f"[ACTIVE] BUY {market.name}: cash insufficient (need {target_price/100:.2f}, "
                    f"have {self._holdings.cash_available/100:.2f}), skipping."
                )
                return
        else:
            asset = self._holdings.assets.get(market)
    ...
    def _cancel_active_slot(self, market_id: int, side: OrderSide, reason: str = ""):
        ...
        Used when a market-side condition makes my standing order stale.
        """
    ...
    def _handle_cash_shortage(self, buy_market: Market, buy_price: int, buy_order: Order, current_performance: float) -> bool:
        ...
        Strategy:
        - Collect all markets (excluding buy_market) where I hold sellable units and there is a live best bid.
        ...
        :param buy_market: The market that I want to buy into.
        ...
        self.inform(f"[Cash Shortage] Submit first SELL: {first_mkt.name} at {first_price/100:.2f}")
